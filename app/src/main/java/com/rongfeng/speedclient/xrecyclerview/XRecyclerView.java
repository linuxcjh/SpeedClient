...
//        return false;
    ...
    /**
     * @param context
     * @param color            分隔线颜色 0：默认背景色
     * @param dp               分隔线高
     * @param leftRightOffsets item左右偏移量
     */
    public void initListAndStly(Context context, int color, int dp, int leftRightOffsets) {
        LinearLayoutManager layoutManager = new LinearLayoutManager(context);
        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
        setLayoutManager(layoutManager);
        setRefreshProgressStyle(ProgressStyle.BallSpinFadeLoader);
        setLaodingMoreProgressStyle(ProgressStyle.Pacman);
        setArrowImageView(R.drawable.refresh_pulldown);
        DividerLine dividerLine = new DividerLine();
        dividerLine.setLeftRightOffsets(Utils.dip2px(context, leftRightOffsets));
        dividerLine.setSize(Utils.dip2px(context, dp));
     ...
     * 临时添加修复筛选后不能加载更多的问题
     */
    public void setResetInfo() {
        previousTotal = 0;
        isnomore = false;
        View footView = mFootViews.get(0);
        if (footView instanceof LoadingMoreFooter) {
            ((LoadingMoreFooter) footView).setState(LoadingMoreFooter.STATE_NO);
        } else {
            footView.setVisibility(View.GONE);
        }
    }
